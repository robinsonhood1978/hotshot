...
------------------------------------------------------------------------------- */
if (!function_exists('maxinet_trx_addons_theme_specific_setup1')) {
	add_filter( 'trx_addons_filter_components_editor', 'maxinet_trx_addons_theme_specific_components');
	function maxinet_trx_addons_theme_specific_components($enable=false) {
		return false;
	}
}

if (!function_exists('maxinet_trx_addons_theme_specific_setup1')) {
	add_action( 'after_setup_theme', 'maxinet_trx_addons_theme_specific_setup1', 1 );
	add_action( 'trx_addons_action_save_options', 'maxinet_trx_addons_theme_specific_setup1', 1 );
	function maxinet_trx_addons_theme_specific_setup1() {
		if (maxinet_exists_trx_addons()) {
			add_filter( 'trx_addons_cv_enable',					'maxinet_trx_addons_cv_enable');
			add_filter( 'trx_addons_demo_enable',				'maxinet_trx_addons_demo_enable');
			add_filter( 'trx_addons_filter_edd_themes_market',	'maxinet_trx_addons_edd_themes_market_enable');
			add_filter( 'trx_addons_cpt_list',					'maxinet_trx_addons_cpt_list');
			add_filter( 'trx_addons_sc_list',					'maxinet_trx_addons_sc_list');
			add_filter( 'trx_addons_widgets_list',				'maxinet_trx_addons_widgets_list');
		}
	}
}

// CV
if ( !function_exists( 'maxinet_trx_addons_cv_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_cv_enable', 'maxinet_trx_addons_cv_enable');
	function maxinet_trx_addons_cv_enable($enable=false) {
		// To do: return false if theme not use CV functionality
		return MAXINET_THEME_FREE
					? false		// Free version
					: true;		// Pro version
	}
}
...
// Demo mode
if ( !function_exists( 'maxinet_trx_addons_demo_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_demo_enable', 'maxinet_trx_addons_demo_enable');
	function maxinet_trx_addons_demo_enable($enable=false) {
...
// EDD Themes market
if ( !function_exists( 'maxinet_trx_addons_edd_themes_market_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_edd_themes_market', 'maxinet_trx_addons_edd_themes_market_enable')) {
	function maxinet_trx_addons_edd_themes_market_enable($enable=false) {
...
// API
if ( !function_exists( 'maxinet_trx_addons_api_list' ) ) {
	//Handler of the add_filter('trx_addons_api_list',	'maxinet_trx_addons_api_list');
	function maxinet_trx_addons_api_list($list=array()) {
...
// CPT
if ( !function_exists( 'maxinet_trx_addons_cpt_list' ) ) {
	//Handler of the add_filter('trx_addons_cpt_list',	'maxinet_trx_addons_cpt_list');
	function maxinet_trx_addons_cpt_list($list=array()) {
...
// Shortcodes
if ( !function_exists( 'maxinet_trx_addons_sc_list' ) ) {
	//Handler of the add_filter('trx_addons_sc_list',	'maxinet_trx_addons_sc_list');
	function maxinet_trx_addons_sc_list($list=array()) {
...
// Widgets
if ( !function_exists( 'maxinet_trx_addons_widgets_list' ) ) {
	//Handler of the add_filter('trx_addons_widgets_list',	'maxinet_trx_addons_widgets_list');
	function maxinet_trx_addons_widgets_list($list=array()) {
...
if ( !function_exists( 'maxinet_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'maxinet_trx_addons_menu_cache');
	function maxinet_trx_addons_menu_cache($list=array()) {
...
if (!function_exists('maxinet_trx_addons_localize_script')) {
	...
	function maxinet_trx_addons_localize_script($arr) {
		$arr['alter_link_color'] = maxinet_get_scheme_color('alter_link');
		return $arr;
	}
}
...
if ( !function_exists( 'maxinet_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'maxinet_trx_addons_sc_type', 10, 2);
	function maxinet_trx_addons_sc_type($list, $sc) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
		if ($sc == 'trx_sc_button') {
			$list['alter'] = esc_html__('Alter', 'maxinet');
			$list['extra'] = esc_html__('Extra', 'maxinet');
		}
		if ($sc == 'trx_sc_price') {
			$list['extra'] = esc_html__('Extra', 'maxinet');
		}
...
if ( !function_exists( 'maxinet_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'maxinet_trx_addons_sc_atts', 10, 2);
	function maxinet_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'maxinet_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'maxinet_trx_addons_sc_map', 10, 2);
	function maxinet_trx_addons_sc_map($params, $sc) {
		...
		$param = array(
			...
			'heading' => esc_html__('Color style', 'maxinet'),
			'description' => wp_kses_data( __('Select color style to decorate this block', 'maxinet') ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label' => true,
			'value' => array_flip(maxinet_get_list_sc_color_styles()),
			'type' => 'dropdown'
		);
...
if ( !function_exists( 'maxinet_trx_addons_sow_map' ) ) {
	add_filter( 'trx_addons_sow_map', 'maxinet_trx_addons_sow_map', 10, 2);
	function maxinet_trx_addons_sow_map($params, $sc) {

		// Param 'color_style'
		$param = array(
			'color_style' => array(
				'label' => esc_html__('Color style', 'maxinet'),
				'description' => wp_kses_data( __('Select color style to decorate this block', 'maxinet') ),
				'options' => maxinet_get_list_sc_color_styles(),
				'default' => 'default',
				'type' => 'select'
			)
		);
		if (in_array($sc, array('trx_sc_button')))
			maxinet_array_insert_after($params, 'size', $param);
		else if (in_array($sc, array('trx_sc_action', 'trx_sc_blogger', 'trx_sc_cars', 'trx_sc_courses', 'trx_sc_content', 'trx_sc_dishes',
								'trx_sc_events', 'trx_sc_form',	'trx_sc_googlemap', 'trx_sc_portfolio', 'trx_sc_price', 'trx_sc_promo',
								'trx_sc_properties', 'trx_sc_services', 'trx_sc_team', 'trx_sc_testimonials', 'trx_sc_title',
								'trx_widget_audio', 'trx_widget_twitter')))
			maxinet_array_insert_after($params, 'title_align', $param);
		return $params;
	}
}

// Add classes to the shortcode's output
if ( !function_exists( 'maxinet_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'maxinet_trx_addons_sc_output', 10, 4);
	function maxinet_trx_addons_sc_output($output, $sc, $atts, $content) {
		...
		if (in_array($sc, array('trx_sc_action'))) {
			...
			if (!empty($atts['color_style']) && !maxinet_is_inherit($atts['color_style']))
...
if ( !function_exists( 'maxinet_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'maxinet_trx_addons_sc_item_title_tag');
	function maxinet_trx_addons_sc_item_title_tag($tag='') {
		return $tag=='h1' ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( !function_exists( 'maxinet_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'maxinet_trx_addons_sc_item_button_args', 10, 3);
	function maxinet_trx_addons_sc_item_button_args($args, $sc, $sc_args) {
...
if ( !function_exists( 'maxinet_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title',	'maxinet_trx_addons_slider_title', 10, 2 );
	function maxinet_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if ( !function_exists( 'maxinet_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles',	'maxinet_trx_addons_sc_googlemap_styles');
	function maxinet_trx_addons_sc_googlemap_styles($list) {
		$list['dark'] = esc_html__('Dark', 'maxinet');
...
if ( !function_exists( 'maxinet_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'maxinet_trx_addons_tiny_mce_style_formats');
	function maxinet_trx_addons_tiny_mce_style_formats($style_formats) {
		...
		if (false && is_array($style_formats) && count($style_formats)>0 ) {
			foreach ($style_formats as $k=>$v) {
				if ( $v['title'] == esc_html__('List styles', 'maxinet') ) {
					$style_formats[$k]['items'][] = array(
								'title' => esc_html__('Arrow', 'maxinet'),
								'selector' => 'ul',
...
if ( !function_exists( 'maxinet_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'maxinet_filter_allow_override_header_image', 'maxinet_trx_addons_allow_override_header_image' );
	function maxinet_trx_addons_allow_override_header_image($allow) {
		return maxinet_is_team_page() ? false : $allow;
	}
}

// Get thumb size for the team items
if ( !function_exists( 'maxinet_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size',	'maxinet_trx_addons_thumb_size', 10, 2);
	function maxinet_trx_addons_thumb_size($thumb_size='', $type='') {
		if ($type == 'team-default')
			$thumb_size = maxinet_get_thumb_size('big');
		return $thumb_size;
	}
}
...
if (!function_exists('maxinet_trx_addons_override_options_fields')) {
	add_filter( 'trx_addons_filter_override_options_fields', 'maxinet_trx_addons_override_options_fields', 10, 2);
	function maxinet_trx_addons_override_options_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'maxinet'),
				"desc" => wp_kses_data( __("Team member's email", 'maxinet') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
