...
if (!function_exists('maxinet_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'maxinet_hovers_theme_setup3', 3 );
	function maxinet_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		maxinet_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'maxinet'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'maxinet') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'maxinet'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'maxinet'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'maxinet') ),
				"std" => 'icon',
				"override" => array(
					'mode' => 'page',
...
if (!function_exists('maxinet_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'maxinet_hovers_theme_setup9', 9 );
	function maxinet_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'maxinet_hovers_frontend_scripts', 1010 );
		add_filter( 'maxinet_filter_localize_script','maxinet_hovers_localize_script' );
		add_filter( 'maxinet_filter_merge_scripts',	'maxinet_hovers_merge_scripts' );
		add_filter( 'maxinet_filter_merge_styles',	'maxinet_hovers_merge_styles' );
		add_filter( 'maxinet_filter_get_css', 		'maxinet_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'maxinet_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'maxinet_hovers_frontend_scripts', 1010 );
	function maxinet_hovers_frontend_scripts() {
		if ( maxinet_is_on(maxinet_get_theme_option('debug_mode')) && maxinet_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'maxinet-hovers', maxinet_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( maxinet_is_on(maxinet_get_theme_option('debug_mode')) && maxinet_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'maxinet-hovers',  maxinet_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('maxinet_hovers_merge_scripts')) {
	//Handler of the add_filter( 'maxinet_filter_merge_scripts', 'maxinet_hovers_merge_scripts' );
	function maxinet_hovers_merge_scripts($list) {
...
if (!function_exists('maxinet_hovers_merge_styles')) {
	//Handler of the add_filter( 'maxinet_filter_merge_styles', 'maxinet_hovers_merge_styles' );
	function maxinet_hovers_merge_styles($list) {
...
if (!function_exists('maxinet_hovers_localize_script')) {
	//Handler of the add_filter( 'maxinet_filter_localize_script','maxinet_hovers_localize_script' );
	function maxinet_hovers_localize_script($arr) {
		$arr['button_hover'] = maxinet_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('maxinet_hovers_add_icons') ) {
	function maxinet_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				if (!is_object($args['cat']) && $product->is_purchasable() && $product->is_in_stock()) {
					maxinet_show_layout(apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'maxinet') : '')
										. '</a>',
										$product));
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'maxinet');
						else
							esc_html_e('Details', 'maxinet');
					}
				?></a>
			</div>
			<?php
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php esc_url(the_permalink()); ?>" aria-hidden="true" class="icon-eye-new"></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'maxinet_hovers_get_css' ) ) {
	//Handler of the add_filter( 'maxinet_filter_get_css', 'maxinet_hovers_get_css', 10, 4 );
	function maxinet_hovers_get_css($css, $colors, $fonts, $scheme='') {
