...
if ( !function_exists('maxinet_need_page_title') ) {
	function maxinet_need_page_title() {
		return !is_front_page() && apply_filters('maxinet_filter_need_page_title', true);
	}
}
...
if ( !function_exists('maxinet_show_layout') ) {
	function maxinet_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('maxinet_get_logo_image') ) {
	function maxinet_get_logo_image($type='') {
		$logo_image = '';
		if (maxinet_is_on(maxinet_get_theme_option('logo_retina_enabled')) && maxinet_get_retina_multiplier() > 1)
			$logo_image = maxinet_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) {
			if (empty($type) && function_exists('the_custom_logo')) {
				...
				$logo_image = maxinet_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
			}
		}
		return maxinet_remove_protocol_from_url($logo_image, false);
	}
}

// Return header video (if set)
if ( !function_exists('maxinet_get_header_video') ) {
	function maxinet_get_header_video() {
		$video = '';
		if (apply_filters('maxinet_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (maxinet_check_theme_option('header_video')) {
				$video = maxinet_get_theme_option('header_video');
...
if ( !function_exists('maxinet_show_post_meta') ) {
	function maxinet_show_post_meta($args=array()) {

		if (is_single() && maxinet_is_off(maxinet_get_theme_option('show_post_meta'))) return ' ';	// Space is need!
		...
		?><div class="post_meta"><?php
			...
			foreach ($components as $comp) {
				...
				// Post categories
				if ($comp == 'categories') {
					$cats = get_post_type()=='post' ? get_the_category_list(', ') : apply_filters('maxinet_filter_get_post_categories', '');
					if (!empty($cats)) {
						?>
						<span class="post_meta_item post_categories"><?php maxinet_show_layout($cats); ?></span>
						<?php
					}

				// Post date
				} else if ($comp == 'date') {
					$dt = apply_filters('maxinet_filter_get_post_date', maxinet_get_date());
					if (!empty($dt)) {
						?>
				...
				// Post counters
					...
					if ( maxinet_exists_trx_addons() && ($output = maxinet_get_post_counters($args['counters'])) != '')
						maxinet_show_layout($output);
					else {
						if (!is_singular() || have_comments() || comments_open()) {
							...
							<a href="<?php echo esc_url(get_comments_link()); ?>" class="post_meta_item post_counters_item post_counters_comments icon-comment-light"><?php
								...
								?><span class="post_counters_label"><?php
									echo esc_html(_n('Comment', 'Comments', $post_comments, 'maxinet'));
								?></span>
							</a>
							<?php
						}
				...
				// Socials share
					...
					maxinet_show_share_links(array(
							'type' => 'drop',
							'caption' => esc_html__('Share', 'maxinet'),
				...
				// Edit page link
					...
					edit_post_link( esc_html__( 'Edit', 'maxinet' ), '<span class="post_meta_item post_edit">', '</span>' );
				}
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('maxinet_show_post_featured') ) {
	function maxinet_show_post_featured($args=array()) {

		$args = array_merge(array(
			'hover' => maxinet_get_theme_option('image_hover'),	// Hover effect
			...
			'seo' => maxinet_is_on(maxinet_get_theme_option('seo_snippets')),
		...
		$thumb_size = !empty($args['thumb_size']) 
						? $args['thumb_size'] 
						: maxinet_get_thumb_size(is_attachment() || is_single() ? 'full' : 'big');
		$post_format = str_replace('post-format-', '', get_post_format());
		$no_image = !empty($args['show_no_image']) ? maxinet_get_no_image() : '';
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), $thumb_size );
				$image = $image[0];
			} else if ($post_format == 'image') {
				$image = maxinet_get_post_image();
				if (!empty($image)) 
					$image = maxinet_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = $no_image;
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . maxinet_add_inline_css_class('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured post_attachment<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>">
					...
					if (!$args['thumb_bg']) 
						echo wp_get_attachment_image( get_the_ID(), $thumb_size, false, 
													 maxinet_is_on(maxinet_get_theme_option('seo_snippets'))
				...
				?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$maxinet_attr = maxinet_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
						<meta itemprop="width" content="<?php echo esc_attr($maxinet_attr[0]); ?>">
						<meta itemprop="height" content="<?php echo esc_attr($maxinet_attr[1]); ?>">
						<?php
					}
					if (!$args['thumb_bg']) {
						if ( has_post_thumbnail() ) {
			...
			if ($has_thumb 
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						maxinet_hovers_add_icons($args['hover']);
					}
					...
					if (($output=maxinet_get_slider_layout(array('thumb_size'=>$thumb_size, 'controls'=>'yes', 'pagination'=>'yes'))) != '')
						maxinet_show_layout($output);
					...
					if (!empty($image)) {
						...
						maxinet_hovers_add_icons($args['hover'], array('image' => $image));
					}
				...
				if (!$args['thumb_only'] && ($post_format == 'video' || $post_format == 'audio')) {
					...
					if ($post_format == 'video') {
						$video = maxinet_get_post_video($post_content, false);
						if (empty($video))
							$video = maxinet_get_post_iframe($post_content, false);
						if (empty($video)) {
							...
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if (!empty($videos) && is_array($videos)) {
									$video = maxinet_array_get_first($videos, false);
								}
							}
						}
						if (!empty($video)) {
							if ( $has_thumb ) {
								$video = maxinet_make_video_autoplay($video);
								?><div class="post_video_hover" data-video="<?php echo esc_attr($video); ?>"></div><?php 
							}
							?><div class="post_video video_frame"><?php 
								if ( !$has_thumb ) {
									maxinet_show_layout($video);
								}
							?></div><?php
						}
					...
					// Put audio over the thumb
						...
						$audio = maxinet_get_post_audio($post_content, false);
						if (empty($audio))
							$audio = maxinet_get_post_iframe($post_content, false);
						if (!empty($audio)) {
							?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php 
								// Add author and title
								$media_author = maxinet_get_theme_option('media_author', '', false, get_the_ID());
								$media_title = maxinet_get_theme_option('media_title', '', false, get_the_ID());
								if ( !empty($media_author) && !maxinet_is_inherit($media_author) ) {
									?><div class="post_audio_author"><?php maxinet_show_layout($media_author); ?></div><?php
								}
								if ( !empty($media_title) && !maxinet_is_inherit($media_title) ) {
									?><h5 class="post_audio_title"><?php maxinet_show_layout($media_title); ?></h5><?php
								}
								// Display audio
								maxinet_show_layout($audio); 
							?></div><?php
						}
					}
				}
...
if ( !function_exists('maxinet_get_no_image') ) {
	function maxinet_get_no_image($no_image='') {
		static $img = '';
		if (empty($img)) {
			$img = maxinet_get_theme_option( 'no_image' );
			if (empty($img)) $img = '';
		}
...
if ( !function_exists('maxinet_add_bg_in_post_nav') ) {
	function maxinet_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = maxinet_get_no_image();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), maxinet_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), maxinet_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		wp_add_inline_style( 'maxinet-main', $css );
	}
}

// Show related posts
if ( !function_exists('maxinet_show_related_posts') ) {
	function maxinet_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
		...
		$taxonomy = $args['post_type'] == 'post' ? 'category' : maxinet_get_post_type_taxonomy();
			...
			<section class="related_wrap">
				<h3 class="section_title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('Related Posts', 'maxinet');
				?></h3>
...
if ( !function_exists('maxinet_show_portfolio_posts') ) {
	function maxinet_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? maxinet_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = maxinet_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = maxinet_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the maxinet_ajax_get_posts action
if ( !function_exists( 'maxinet_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_maxinet_ajax_get_posts',			'maxinet_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_maxinet_ajax_get_posts',	'maxinet_ajax_get_posts_callback');
	function maxinet_ajax_get_posts_callback() {
		...
		if ($id > 0) {
			...
			maxinet_storage_set('blog_mode', 'blog');
			maxinet_storage_set('options_meta', get_post_meta($id, 'maxinet_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => maxinet_show_portfolio_posts(array(
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'maxinet');
		}
...
// Show pagination
if ( !function_exists('maxinet_show_pagination') ) {
	function maxinet_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = maxinet_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'maxinet' ),
				'next_text' => esc_html__( '>', 'maxinet' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'maxinet' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'maxinet'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'maxinet') : esc_html__('Newest posts', 'maxinet') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'maxinet') : esc_html__('Older posts', 'maxinet'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'maxinet_single_comments_field' ) ) {
    function maxinet_single_comments_field( $args ) {
