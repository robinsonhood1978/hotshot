...
class Custom_Woocommerce_Subscription_Status
{
    ...
    public function run()
    {
        ...
        add_filter('woocommerce_can_subscription_be_updated_to_switching', array($this, 'enable_switching_in_new_statuses'), 100, 2);
    ...
    /**
     * Registered new status by adding `Pending Active` to $registered_statuses array.
     *
     * @access public
     *
    ...
    {
        $registered_statuses['wc-pending-active'] = _nx_noop('Pending Active <span class="count">(%s)</span>', 'Pending Active <span class="count">(%s)</span>', 'post status label including post count', 'custom-wcs-status-texts');
        $registered_statuses['wc-switching'] = _nx_noop('Switching <span class="count">(%s)</span>', 'Switching <span class="count">(%s)</span>', 'post status label including post count', 'custom-wcs-status-texts');
        return $registered_statuses;
    }

    /**
     * Add new status `Pending Active` to $subscription_statuses array.
     *
     * @access public
     *
    ...
    {
        $subscription_statuses['wc-pending-active'] = _x('Pending Active', 'Subscription status', 'custom-wcs-status-texts');
        $subscription_statuses['wc-switching'] = _x('Switching', 'Subscription status', 'custom-wcs-status-texts');
    ...
    {
        if ( $new_status == 'pending-active' || $new_status == 'switching' ) {
     ...
     * This code will also activate `reactivate` link in the list page for the subscription with `Pending Active` status
     *
     * @access public
     *
     * @param boolean $can_be_updated default value if the current subscription can be updated to new status or not.
    ...
    {
        if ($subscription->payment_method_supports('subscription_reactivation') && $subscription->has_status(array('on-hold', 'pending-active'))) {
            $can_be_updated = true;
     ...
     * This function replaces the default code with the new one.
     * This code will also activate `suspend` link in the list page for the subscription with `Pending Active` status
     *
     * @access public
     *
     * @param boolean $can_be_updated default value if the current subscription can be updated to new status or not.
    ...
    {
        if ($subscription->payment_method_supports('subscription_suspension') && $subscription->has_status(array('active', 'pending', 'pending-active'))) {
            $can_be_updated = true;
        } else {
            $can_be_updated = false;
        }
    ...
    /**
     * Enable `Switching` status in the status change dropdown of the subcription with this new status.
     * This function replaces the default code with the new one.
     *
     * @access public
     *
     ...
     * @return boolean $can_be_updated If the current subscription can be updated to new status or not.
     */
    public function enable_switching_in_new_statuses($can_be_updated, $subscription)
    {
    ...
    {
        $bulk_actions['pending-active'] = _x('Mark Pending Active', 'an action on a subscription', 'custom-wcs-status-texts');
        return $bulk_actions;
    }
