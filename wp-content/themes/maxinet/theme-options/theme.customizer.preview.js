...
( function( $ ) {
	...
	var $style = $('#maxinet-customizer-inline-css'),
		api = wp.customize;

	// Prepare inline styles in the preview window
	if ( $style.length == 0 ) {
		var style_container = '<'+'style type="text/css" id="maxinet-customizer-inline-css" />',
			$theme_css = $('#maxinet-main-inline-css');
		if ($theme_css.length == 0)
			$theme_css = $('#maxinet-main-css');
		if ($theme_css.length == 0)
			$('head').append(style_container);
		else
			$theme_css.after(style_container);
		$style = $('#maxinet-customizer-inline-css');
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-other-controls', function( obj ) {
			...
			if (id.indexOf('body_style') == 0) {
				...
				$('.sc_layouts_logo .logo_text').html( maxinet_prepare_macros(val) );

			} else if (id == 'blogdescription') {
				$( '.sc_layouts_logo .logo_slogan' ).html( maxinet_prepare_macros(val) );

			} else if (id == 'copyright') {
				$( '.copyright_text' ).html( maxinet_prepare_macros(val) );
				...
				if (id == 'front_page_bg_image') {
					...
					if (color)
						$('.front_page_section_'+section+'_inner').css('background-color', mask == 1 ? color : maxinet_hex2rgba(color, mask));
					...
					$('.'+id.replace('front_page_', 'front_page_section_').replace('_fullheight', '_inner'))
							.height('auto')
							.toggleClass('maxinet-full-height sc_layouts_flex sc_layouts_columns_middle', val);
					$(window).trigger('resize');
				}
			}
		} );
