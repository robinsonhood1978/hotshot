/* global maxinet_color_schemes, maxinet_dependencies, Color */
...
( function( api ) {
	...
	for (var i in maxinet_color_schemes) {
		cssTemplate[i] = wp.template( 'maxinet-color-scheme-'+i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'maxinet-fonts' );
	
	// Set initial state of controls
	api.bind('ready', function() {

		// Add 'reset' button
		jQuery('#customize-header-actions #save').before('<input type="button" class="button customize-action-reset" value="'+maxinet_customizer_vars['msg_reset']+'">');
		jQuery('#customize-header-actions .customize-action-reset').on('click', function(e) {
			if (confirm(maxinet_customizer_vars['msg_reset_confirm'])) {
		...
		jQuery('#customize-theme-controls [id^="customize-control-load_fonts"]').on('focusout', maxinet_customizer_update_load_fonts);		

		// Click on the actions button
		jQuery('#customize-theme-controls .control-section .customize-control-button input[type="button"]').on('click', maxinet_customizer_actions);

		// Check dependencies in the each section on ready
		jQuery('#customize-theme-controls .control-section').each(function () {
			maxinet_customizer_check_dependencies(jQuery(this));
		});
		...
		jQuery('#customize-theme-controls .control-section > .accordion-section-title').on('click', function() {
			...
			if (id!='') {
				...
				if (section.length > 0) maxinet_customizer_check_dependencies(section);
			}
		});
	...
	api.bind('change', function(obj) {
		maxinet_customizer_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		maxinet_customizer_refresh_preview(obj);
	});

	// Return value of the control
	function maxinet_customizer_get_field_value(fld) {
	...
	function maxinet_customizer_check_dependencies(cont) {
		cont.find('.customize-control').each(function() {
			...
			var fld=null, val='', i;
			var depend = false;
			for (fld in maxinet_dependencies) {
				if (fld == id) {
					depend = maxinet_dependencies[id];
					break;
				}
			}
			if (depend) {
				...
				for (i in depend) {
					...
					if (fld.length > 0) {
						val = maxinet_customizer_get_field_value(fld);
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					ctrl.show().removeClass('maxinet_options_no_use');
				} else {
					ctrl.hide().addClass('maxinet_options_no_use');
				}
			}
			...
			if (id == 'color_scheme') {		// Disable color schemes less then main scheme!
				...
				if (fld.length > 0) {
					val = maxinet_customizer_get_field_value(fld);
					var num = 0;
					for (i in maxinet_color_schemes) {
						num++;
						if (i == val) break;
					}
					cont.find('.customize-control').each(function() {
	...
	function maxinet_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			// Any theme fonts parameter is changed
				...
				for (opt in maxinet_theme_fonts) {
					for (rule in maxinet_theme_fonts[opt]) {
						if (opt+'_'+rule == id) {
							// Store new value in the color table
							maxinet_customizer_update_theme_fonts(opt, rule, val);
							processed = true;
							break;
						}
					}
			...
			// Refresh CSS
			if (processed) maxinet_customizer_update_css();
		}
	...
	// Actions buttons
	function maxinet_customizer_actions(e) {
	...
	function maxinet_customizer_update_theme_fonts(opt, rule, value) {
		maxinet_theme_fonts[opt][rule] = value;
	}
	
	// Change theme fonts options if load fonts is changed
	function maxinet_customizer_update_load_fonts() {
		var opt_list=[], i, tag, sel, opt, name='', family='', val='', new_val = '', sel_idx = 0;
		updateCSS = false;
		for (i=1; i <= maxinet_customizer_vars['max_load_fonts']; i++) {
			name = api('load_fonts-'+i+'-name')();
			if (name == '') continue;
			family = api('load_fonts-'+i+'-family')();
			opt_list.push([name, family]);
		}
		for (tag in maxinet_theme_fonts) {
	...
	function maxinet_customizer_update_css() {
		...
		var fonts = maxinet_customizer_add_theme_fonts(maxinet_theme_fonts);

		// Make styles and add into css
		css += maxinet_customizer_prepare_html_value(cssTemplate['theme_fonts']( fonts ));

		// Add colors
		for (var scheme in maxinet_color_schemes) {
			
			var i, colors = [];
			
			// Copy all colors!
			for (i in maxinet_color_schemes[scheme].colors) {
				colors[i] = maxinet_color_schemes[scheme].colors[i];
			}
            ...
            var tmpl = jQuery( '#tmpl-maxinet-color-scheme-' + scheme ).html().trim();
            for (i in colors) {
	...
	function maxinet_customizer_add_theme_colors(colors) {
		if (maxinet_additional_colors) {
			var clr = '', v = '';
			for (var k in maxinet_additional_colors) {
				v = maxinet_additional_colors[k];
				clr = colors[v['color']];
		...
		for (var tag in fonts) {
			...
			rez[tag+'_font-size'] = typeof fonts[tag]['font-size'] != 'undefined' 
									&& fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + maxinet_customizer_prepare_css_value(fonts[tag]['font-size']) + ";"
												: '';
			...
			rez[tag+'_margin-top'] = typeof fonts[tag]['margin-top'] != 'undefined' 
									...
									&& fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + maxinet_customizer_prepare_css_value(fonts[tag]['margin-top']) + ";"
												: '';
			rez[tag+'_margin-bottom'] = typeof fonts[tag]['margin-bottom'] != 'undefined' 
									&& fonts[tag]['margin-bottom'] != '' 
									&& fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + maxinet_customizer_prepare_css_value(fonts[tag]['margin-bottom']) + ";"
												: '';
		}
		return rez;
	}
	...
	// Add ed to css value
	function maxinet_customizer_prepare_css_value(val) {
		if (val != '' && val != 'inherit') {
			var ed = val.substr(-1);
			if ('0'<=ed && ed<='9') val += 'px';
		}
	...
	function maxinet_customizer_prepare_html_value(val) {
