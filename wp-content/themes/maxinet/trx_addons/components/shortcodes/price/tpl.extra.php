			...
			?><div class="sc_price_item_info">
				...
				if (!empty($item['image'])) {
					...
					if (!empty($item['image'])) {
						...
						$alt_title = esc_html__('sc-price-image', 'maxinet');
						?><div class="sc_price_item_image"><img src="<?php echo esc_url($item['image']); ?>" alt="<?php echo esc_attr($alt_title); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
					}
				} else {
				...
				<?php
					...
					if (!empty($item['link']) && !empty($item['link_text'])) {
						?><a href="<?php echo esc_url($item['link']); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_price_item_link sc_button sc_button_size_small', 'sc_price', $args, $item)); ?>" target="_blank"><?php
