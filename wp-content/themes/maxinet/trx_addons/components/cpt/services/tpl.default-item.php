...
?>>
		...
		if ( has_post_thumbnail() && $args['featured']=='image') {
			trx_addons_get_template_part('templates/tpl.featured.php',
											...
											apply_filters('trx_addons_filter_args_featured', array(
																...
																'hover' => 'zoomin',
																'thumb_size' => apply_filters('trx_addons_filter_thumb_size', maxinet_get_thumb_size($args['columns'] > 2 ? 'extra' : 'extra'), 'services-default'),
			...
			?>><?php
				if (!empty($svg)) {
					...
					$alt_title = esc_html__('sc-services-image', 'maxinet');
					?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="<?php echo esc_attr($alt_title); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
				}
			?></a><?php
		} else {
	...
	<div class="sc_services_item_info">
		<div class="sc_services_item_header">
			...
			?></a></h4>
			<div class="sc_services_item_subtitle"><?php ?></div>
		</div>
		<?php if (!isset($args['hide_excerpt']) || $args['hide_excerpt']==0) { ?>
			<div class="sc_services_item_content"><?php the_excerpt(); ?></div>
			<div class="sc_services_item_button sc_item_button"><a href="<?php echo esc_url($link); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_button sc_button_size_normal', 'sc_services', $args)); ?>"><?php esc_html_e('More Info', 'maxinet'); ?></a></div>
		<?php } ?>
	</div>
</div>
<?php
