 ...
 * Plugin URI: https://opmc.co.nz/product/payment-express-dps-pxpay-pxpost-payment-gateway-for-woocommerce-supports-woocommerce-subscriptions/
 * Description: Allows your customers to pay via Payment Express PxPay (Supports PxPay1 & 2). The extension also uses PxPost for recurring billing support with the WooCommerce Subscriptions extension. Tested on WordPress v4.8 with WooCommerce v3.0.9. Refunds should be processed via your Payment Express Merchant login.
 * Version: 2.3
 * Author: OPMC
 * Author URI: http://opmc.com.au/
 * Developer: OPMC
 * Developer URI: http://opmc.com.au/
 * Tested on WooCommerce: 3.0.9
...
self::wc_pxhybrid_log( array( "Data sent to pxpost for refund >>>" => $cmdDoTxnTransaction ) );
			 ...
			 */
			public static function wc_pxpost_pxpost_parse_xml( $data ) {
			...
			public function wc_actions_column_refund_button_function( $actions, $the_order ) {
				...
				if ( ! ( in_array( $the_order->get_status(), array( 'refunded' ) ) ) and $the_order->get_payment_method_title() == 'Payment Express' ){
