...
class WC_Gateway_Payment_Express_Hybrid extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_scheduled_subscription_payment_' . $this->id, array( $this, 'process_payment_for_scheduled_subscription' ), 10, 3 );
	...
	public function process_payment_for_scheduled_subscription( $amount, $order, $product_id='' ) {
		...
		$userSuppliers = get_user_meta( $order->get_user_id(), '_supDpsBillingID', true ); 	/* This is actually a BillingId */
		$merchRef = 'Scheduled auto payment for order '.$order->get_id();
		$currency = get_woocommerce_currency(); 
		$currency = apply_filters( 'filter_pxpost_reccurring_currency', $currency, $order, $this->settings );

		$name = '';
		$this->log->add( 'pxpost', print_r( array( 'userID and BillingId' => array( 'userid' => $order->get_user_id(), 'BillingId' => $userSuppliers ) ), true ) );
		...
		if ( preg_match("/APPROVED/i", $dpsCardHolderResponseText ) ) {
			...
			add_post_meta( $order->get_id(), 'dpsTxnRef', $dpsTxnRef, true );
